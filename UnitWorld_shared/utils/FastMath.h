...
public:
	...
	static unsigned long long isolateMostSignificantBit(unsigned long long n)
	{
		n |= (n >> 1);
		n |= (n >> 2);
		n |= (n >> 4);
	...
	static unsigned long long isolateLeastSignificantBit(const unsigned long long& n)
	{
		return n & (0 - n);
	}

	static unsigned char singleBitToPosition(const unsigned long long& n)
	{
		return debruijn64Table[(n * 0x03f79d71b4cb0a89) >> 58] + (unsigned char)(n != 0);
	}

	static unsigned char positionOfMostSignificantBit(unsigned long long n)
	{
		return singleBitToPosition(isolateMostSignificantBit(n));
	}

	static unsigned char positionOfLeastSignificantBit(const unsigned long long& n)
	{
		return singleBitToPosition(isolateLeastSignificantBit(n));
	}

	static unsigned long long positionToSingleBit(const unsigned char& n)
	{
		return (unsigned long long)1 << n - 1;
	}

	static unsigned char bitBoardPopulationCount(unsigned long long n)
	{
	...
	static unsigned long long nextOrSamePrime(const unsigned long long n)
	{
