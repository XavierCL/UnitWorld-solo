...
class Clusterer:
    def __init__(self):
        ...
        self.clusterer = DBSCAN(eps=100, min_samples=25)
    ...
    def cluster(self, ids: np.ndarray, positions: np.ndarray) -> Tuple[np.ndarray, np.ndarray]:
        ...
        singuityClusterIds = self.clusterer.fit_predict(positions)
        ...
        for oldClusterId in range(np.max(lastClusters) + 1):
            oldClusterNewAndUnclusteredIds = singuityClusterIds[lastClusters == oldClusterId]
            oldClusterNewIds = oldClusterNewAndUnclusteredIds[oldClusterNewAndUnclusteredIds != -1]
        ...
        postClusters = assign(singuityClusterIds, lastClusters != -1, oldClusterToNewCluster[lastClusters[lastClusters != -1]])
        ...
        oldClusterMapping = assign(
            np.ones(0 if len(uniqueLastClusters) == 0 else np.max(uniqueLastClusters) + 1, dtype=np.int32) * -1,
            uniqueLastClusters,
        ...
        return oldClusterMapping, reducedPostClusters
